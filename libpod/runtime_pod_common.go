...
func (r *Runtime) removePod(ctx context.Context, p *Pod, removeCtrs, force bool, timeout *uint) (map[string]error, error) {
	...
	for volName := range ctrNamedVolumes {
		...
		if err := r.removeVolume(ctx, volume, false, timeout, false, false); err != nil {
			...
			if errors.Is(err, define.ErrNoSuchVolume) || errors.Is(err, define.ErrVolumeRemoved) || errors.Is(err, define.ErrVolumeBeingUsed) || errors.Is(err, define.ErrVolumePinned) {
				continue
			}
