...
func (r *Runtime) RemoveContainersForImageCallback(ctx context.Context, force bool) libimage.RemoveContainerFunc {
	return func(imageID string) error {
		...
		for _, vol := range vols {
			...
			// using it.
			if err := r.RemoveVolume(ctx, vol, true, nil, false); err != nil {
				if errors.Is(err, define.ErrVolumePinned) {
					return fmt.Errorf("removing image %s: volume %s is pinned; unpin it first with 'podman volume pin --unpin %s': %w", imageID, vol.Name(), vol.Name(), err)
				}
